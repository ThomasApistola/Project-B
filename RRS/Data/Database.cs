...
public static class Database {
    ...
    public static bool Insert(Accounts account) {
        if (account is not null) {
            //Creating a connection to the database
            SqliteConnection db_conn = CreateConn();
            // AddColumnsIfNeeded(db_conn);

            SqliteCommand sqlite_cmd;
            sqlite_cmd = db_conn.CreateCommand();
            sqlite_cmd.CommandText = $"INSERT INTO Accounts (Email, Password, FirstName, LastName, Gender, Age, PhoneNumber, Language, AccountLevel) VALUES ('{account.Email}', '{account.GetPassword()}', '{account.FirstName}', '{account.LastName}','{account.Gender}', '{account.Age}', '{account.PhoneNumber}', 'EN', {account.AccountLevel});";
            sqlite_cmd.ExecuteNonQuery();
...
//     public static void AddColumnsIfNeeded(SqliteConnection db_conn)
// {
//      using (var cmd = new SqliteCommand("PRAGMA table_info(Accounts);", db_conn))
...
//         while (reader.Read())
//         {
//             if (reader["name"].ToString() == "Gender")
//             {
//                 genderExists = true;
//             }
...
//         if (!genderExists)
//         {
//             using (var addGenderCmd = new SqliteCommand("ALTER TABLE Accounts ADD COLUMN Gender TEXT;", db_conn))
//             {
//                 addGenderCmd.ExecuteNonQuery();
//             }
//         }
...
//         if (!ageExists)
//         {
//             using (var addAgeCmd = new SqliteCommand("ALTER TABLE Accounts ADD COLUMN Age INTEGER;", db_conn))
//             {
//                 addAgeCmd.ExecuteNonQuery();
//         }
//     }}}
    ...
    public static Accounts SelectAccount(string email, string password) {
        ...
        if (db_conn is not null) {
            ...
            {                
                ...
                string AccountPassword = sqlite_datareader.GetString(2);
                string AccountFirstName = sqlite_datareader.GetString(3);
                string AccountLastName = sqlite_datareader.GetString(4);
                string AccountGender = sqlite_datareader.GetString(5);
                int AccountAge = sqlite_datareader.GetInt32(6);
                string AccountPhoneNumber = sqlite_datareader.GetString(7);
                string AccountLanguage = sqlite_datareader.GetString(8);
                int AccountAccountLevel = sqlite_datareader.GetInt32(9);

                CloseConn(db_conn);
                return new Accounts(AccountID, AccountEmail, AccountFirstName, AccountLastName, AccountGender, AccountAge, AccountPhoneNumber, AccountLanguage, AccountAccountLevel);
            }
        }
        return null;
    }
    ...
    public static List<Accounts> SelectAccount() {
        ...
        if (db_conn is not null) {
            ...
            {                
                ...
                string AccountFirstName = sqlite_datareader.GetString(3);
                string AccountLastName = sqlite_datareader.GetString(4);
                string AccountGender = sqlite_datareader.IsDBNull(5) ? string.Empty : sqlite_datareader.GetString(5);
                int AccountAge = sqlite_datareader.IsDBNull(6) ? 0 : sqlite_datareader.GetInt32(6);
                string AccountPhoneNumber = sqlite_datareader.GetString(7);
                string AccountLanguage = sqlite_datareader.GetString(8);
                int AccountAccountLevel = sqlite_datareader.GetInt32(9);

                results.Add(new Accounts(AccountID, AccountEmail, AccountFirstName, AccountLastName, AccountGender, AccountAge, AccountPhoneNumber, AccountLanguage, AccountAccountLevel));
            }
            CloseConn(db_conn);
            return results;
        }
    ...
    public static List<Reservations> SelectReservations(int restaurantID, Reservations reservation) {
        SqliteConnection db_conn = CreateConn();
        ...
        if (db_conn is not null) {
            ...
            SqliteCommand sqlite_cmd;
            sqlite_cmd = db_conn.CreateCommand();
            sqlite_cmd.CommandText = $"SELECT * FROM Reservations WHERE RestaurantID = {restaurantID} AND TimeSlotID = {reservation.TimeSlotID} AND TableID = {reservation.TableID}";
    ...
    //Select Everything
    public static List<Table> SelectTable() {
        SqliteConnection db_conn = CreateConn();
        ...
        if (db_conn is not null) {
            ...
            while (sqlite_datareader.Read()) {
                ...
                returnValue.Add(new Table(tableID, tableRestauratnID, tableName, tableMaxSize));
            }
    ...
    public static List<Reservations> SelectReservationsForTableAndTimeSlot(int timeSlotID, int tableID) {
    SqliteConnection db_conn = CreateConn();
    List<Reservations> results = new List<Reservations>();

    if (db_conn is not null) {
        SqliteDataReader sqlite_datareader;
        SqliteCommand sqlite_cmd = db_conn.CreateCommand();
        sqlite_cmd.CommandText = $"SELECT * FROM Reservations WHERE TimeSlotID = {timeSlotID} AND TableID = {tableID}";

        sqlite_datareader = sqlite_cmd.ExecuteReader();
        while (sqlite_datareader.Read()) {
                int reservationID = sqlite_datareader.GetInt32(0);
                int restaurantID = sqlite_datareader.GetInt32(1);
                int TimeSlotID = sqlite_datareader.GetInt32(2);
                int TableID = sqlite_datareader.GetInt32(3);
                int accountID = sqlite_datareader.GetInt32(4);
                int status = sqlite_datareader.GetInt32(5);
                
                results.Add(new Reservations(reservationID, restaurantID, TimeSlotID, TableID, accountID, status));
               } 
        CloseConn(db_conn);     
        return results;}
    ...
    //     sqlite_cmd.Parameters.AddWithValue("@TimeSlotID", timeSlotID);
    //     sqlite_cmd.Parameters.AddWithValue("@TableID", tableID);
    ...
    //Select Everything for a certain Restaurant
    public static List<Table> SelectTableForRestaurant(int RestaurantID) {
        SqliteConnection db_conn = CreateConn();
        ...
        if (db_conn is not null) {
            ...
            SqliteCommand sqlite_cmd;
            sqlite_cmd = db_conn.CreateCommand();
            sqlite_cmd.CommandText = $"SELECT * FROM Tables WHERE RestaurantID = {RestaurantID}";
    ...
    public static Table SelectTable(int TableID) {
        ...
        if (db_conn is not null) {
            ...
            SqliteCommand sqlite_cmd;
            sqlite_cmd = db_conn.CreateCommand();
            sqlite_cmd.CommandText = $"SELECT * FROM Tables WHERE ID = {TableID}";
            ...
            while (sqlite_datareader.Read())
            {
                ...
                return new Table(tableID, tableRestauratnID, tableName, tableMaxSize);;
            }
        }
        return null;
    }

    //Select for a certain Restaurant by ID
    public static List<Table> SelectTable(int TableID, int RestaurantID) {
        SqliteConnection db_conn = CreateConn();
        ...
        if (db_conn is not null) {
            ...
            SqliteCommand sqlite_cmd;
            sqlite_cmd = db_conn.CreateCommand();
            sqlite_cmd.CommandText = $"SELECT * FROM Tables WHERE ID = {TableID} AND RestaurantID = {RestaurantID}";
    ...
    public static int GetOpenPositions(int tableID, int timeSlotID, int restaurantID) {
        SqliteConnection db_conn = CreateConn();
        ...
        if (db_conn is not null) {
            ...
            SqliteCommand sqlite_cmd;
            sqlite_cmd = db_conn.CreateCommand();
            sqlite_cmd.CommandText = $"SELECT * FROM Tables WHERE RestaurantID = {restaurantID} AND ID = {tableID}";

            sqlite_datareader = sqlite_cmd.ExecuteReader();

            int tableMaxSize = 0;
            int ReservationsForTable = 0;
            while (sqlite_datareader.Read()) {
                tableMaxSize = sqlite_datareader.GetInt32(3);
                ReservationsForTable = RowCount("Reservations", $"TimeSlotID = {timeSlotID} AND TableID = {tableID}");
            }

            CloseConn(db_conn);
            return tableMaxSize - ReservationsForTable;
        }
    ...
    public static int SelectSharedInfoCode (int AccountIDOne, int AccountIDTwo) {
        SqliteConnection db_conn = CreateConn();
        ...
        if (db_conn is not null) {
            ...
            SqliteCommand sqlite_cmd;
            sqlite_cmd = db_conn.CreateCommand();
            sqlite_cmd.CommandText = $"SELECT Confirmation FROM InfoShared WHERE AccountID_One = {AccountIDOne} AND AccountID_Two = {AccountIDTwo} OR AccountID_One = {AccountIDTwo} AND AccountID_Two = {AccountIDOne}";

            sqlite_datareader = sqlite_cmd.ExecuteReader();
            while (sqlite_datareader.Read())
            {
                results = sqlite_datareader.GetInt32(0);
            }
    ...
    public static bool UpdateShareInfo(int ShareToAccount, int ShareFromAccount, int InfoShareCode) {
        //Creating a connection to the database
        SqliteConnection db_conn = CreateConn();

        SqliteCommand sqlite_cmd;
        sqlite_cmd = db_conn.CreateCommand();
        sqlite_cmd.CommandText = $"UPDATE InfoShared SET Confirmation = {InfoShareCode} WHERE AccountID_One = {ShareToAccount} AND AccountID_Two = {ShareFromAccount} OR AccountID_One = {ShareFromAccount} AND AccountID_Two = {ShareToAccount};";
        sqlite_cmd.ExecuteNonQuery();
